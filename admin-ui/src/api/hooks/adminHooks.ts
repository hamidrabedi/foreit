...
export function useMetadata(options?: Omit<UseQueryOptions<MetadataResponse>, 'queryKey' | 'queryFn'>) {
  return useQuery({
...
export function useModels(options?: Omit<UseQueryOptions<MetadataResponse>, 'queryKey' | 'queryFn'>) {
...
export function useUpdateObject<T = any>(
  model: string,
  options?: UseMutationOptions<T, Error, { id: string | number; data: Partial<ModelFormData> }>
) {
  const queryClient = useQueryClient();
