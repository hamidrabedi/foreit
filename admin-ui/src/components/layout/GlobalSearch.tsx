...
export function GlobalSearch() {
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsLoading(true);
      try {
        const res = await axiosInstance.get("/admin/api/v1/search", {
          params: { q: query }
        });
  ...
  const handleSelect = (item: any, model: string) => {
    const path = item.url;
    addTab({ id: `search-${model}-${item.id}`, title: `${model}: ${item.title}`, path });
    navigate({ to: path });
    setOpen(false);
    setQuery("");
  };
            ...
            >
              ...
              <div className="max-h-[60vh] overflow-y-auto">
                {!query && (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-sm font-bold text-foreground/80">Search Forge Models</p>
                    <p className="text-xs text-muted-foreground mt-1">Start typing to see instantly correlated records</p>
                  </div>
                )}
                ...
                {results.map((group) => (
                  <div key={group.model} className="p-2">
                    ...
                    {group.items.map((item: any) => (
                      <button
                        key={item.url}
                        onClick={() => handleSelect(item, group.model)}
              ...
              <div className="p-3 bg-muted/20 border-t border-border/10 flex items-center justify-between">
                 ...
                 <div className="flex gap-2">
                    <span className="text-[10px] bg-background px-1.5 py-0.5 rounded border border-border/50 text-muted-foreground">ESC to close</span>
                 </div>
              </div>
            </div>
          </div>
