...
export default function ModelFormPage({ mode }: ModelFormPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate({ to: `/admin/${modelName}` });
    } catch (error) {
      console.error("Failed to save:", error);
      const errorMsg = (error as any)?.response?.data?.message || "Failed to save changes. Please try again.";
      toast({
        title: "Error",
        ...
        <FormHeader className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Button
              ...
              onClick={() => navigate({ to: `/admin/${modelName}` })}
            >
