...
| #230 | Waves 1-2: dead code, tests, duplicates | §4, §5, §7, B8, B14, B30, B36 | tests: 4 tautological removed, 15 skips removed, missing assertions; one viewset (`BaseViewSet` + auth/permission/throttle classes); unreferenced filter code removed; registry admin/API plugins fail loudly; `forge test` runs `go test`; one rate-limit store (`internal/ratelimit`); one API error format (RFC 7807, **response body change**); checksum, test helpers, settings durations, `ErrUserNotFound` merged; CI misspell fixed | in review (ready; restores done, intentional breaks listed below) |
| #231 | Wave 3: design fixes, file splits, compatible APIs | D1, D3, D4, D5, D7, D8, D12, D13, B10, B11, B29 | config loaded once; `MustNewManager`; ORM transactions (`Manager.WithTx`); dialect-owned placeholders; one expression rendering path (deprecated API kept); relation columns resolved once (proven equivalent); CSRF for cookie sessions; four god files split (router and queryset verified pure moves); three complex functions broken up with characterization tests; original exported signatures kept with new `...WithConfig`/`...ForDriver`/`...ForPK`/`...Tx` functions | in review (merge after #230) |
...
| `execute.NewExecutor` compatibility wrapper hard-coded the postgres migrate driver instead of reading the configured driver | Wave 3 branch | reads config again (#231) |
| Old-signature `orm.ExecuteInsert/BulkInsert/Update/Delete(*db.DB)` stopped rebinding `$N` placeholders for SQLite | Wave 3 branch | rebind restored, literal-safe (#231) |
...
2. Wave 3: done in #231. Merge #230 first, then rebase #231: keep both sides of `db/dialect/dialect_test.go`, take #230's deletion of `db/migrate/execute/executor.go`. After that the refactor plan is complete: Wave 4 (library swaps) is documented only, and D2, D6, D9, D10 (Atlas), D11 and D14 are recorded as decisions for the owner.
