...
export class AdminAPIClient {
  ...
  constructor(baseURL: string = getAdminConfig().apiBase) {
    ...
    this.client.interceptors.response.use(
      ...
      (error: AxiosError<ErrorResponse>) => {
        if (error.response?.status === 401) {
          ...
          window.location.href = `${loginPath}?redirect=${redirectTo}`;
        }
  ...
  // History
  async getHistory(model: string, id: string | number): Promise<HistoryEntry[]> {
    const response = await this.client.get(`/${model}/${id}/history`);
    return response.data;
  }

  getExportURL(model: string, format: string = "json"): string {
    return `${this.baseURL}/${model}/export?format=${encodeURIComponent(format)}`;
  }
}
