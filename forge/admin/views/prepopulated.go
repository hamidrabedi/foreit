...
// PrepopulatedFieldHandler handles auto-population of fields for the new admincore system
type PrepopulatedFieldHandler[T any] struct {
	admin *admin.Admin[T]
}

// NewPrepopulatedFieldHandler creates a new prepopulated field handler
func NewPrepopulatedFieldHandler[T any](admin *admin.Admin[T]) *PrepopulatedFieldHandler[T] {
...
func (h *PrepopulatedFieldHandler[T]) PopulateFields(ctx context.Context, instance *T, formData admin.FormData, isNew bool) error {
	...
	if config.GetPrepopulatedFields != nil {
		prepopulatedFields = config.GetPrepopulatedFields(ctx, instance, isNew)
	}
