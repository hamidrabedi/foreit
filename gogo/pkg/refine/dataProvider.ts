/**
 * Refine Data Provider for Gogo Admin Engine
 * 
 * This data provider connects Refine to the Gogo admin engine REST API.
 * It implements Refine's DataProvider interface for seamless integration.
 */
...
/**
 * Creates a Refine data provider for Gogo admin engine
 */
      ...
      if (filters && filters.length > 0) {
        filters.forEach((filter) => {
          if ("field" in filter && "operator" in filter && "value" in filter) {
            ...
            // Map Refine operators to Gogo API format
            switch (operator) {
              case "eq":
