...
'use strict';
{
    ...
    function updateCounter(actionCheckboxes, options) {
        ...
        const actions_icnt = Number(counter.dataset.actionsIcnt || 0);
        counter.textContent = sel + ' of ' + actions_icnt + ' selected';
        ...
        if (indexButton) {
            indexButton.addEventListener('click', function(event) {
                if (list_editable_changed) {
                    const confirmed = confirm("You have unsaved changes on individual editable fields. If you run an action, your unsaved changes will be lost.");
                    if (!confirmed) {
                        event.preventDefault();
                    }
                }
