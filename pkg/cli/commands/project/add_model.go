...
func (c *AddModelCommand) Execute(ctx *cmd.Context, args []string) error {
	modelName := args[0]
...
func detectAppName(ctx *cmd.Context, projectRoot string) (string, error) {
	// Check flag first
...
func registerModelInAdmin(adminPath, appName, modelName string) error {
	...
	// Check if model is already registered (using v2 admin system)
	...
	if !strings.Contains(contentStr, registration) {
		// Add import if needed (using v2 admin system)
		if !strings.Contains(contentStr, "github.com/forgego/forge/pkg/admin") {
