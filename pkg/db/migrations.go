...
import (
	...
	"github.com/forgego/forge/pkg/migrate/apply"
...
func (mr *MigrationRunner) validatePendingMigrations(ctx context.Context, currentVersion uint) error {
	...
	validator := apply.NewMigrationValidator()
...
func (mr *MigrationRunner) GetDetailedStatus(ctx context.Context) (*DetailedMigrationStatus, error) {
	...
	reporter := apply.NewStatusReporter(mr.migrationsPath, mr.migrate, mr.db.DB)
	...
	// Convert apply.DetailedStatus to db.DetailedMigrationStatus
