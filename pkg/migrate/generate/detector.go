...
type ChangeDetector interface {
	...
	DetectChanges(current, previous []*generator.ModelDefinition) ([]types.Change, error)
}
...
// DetectChanges compares current models to previous state and returns all changes
func (d *Detector) DetectChanges(current, previous []*generator.ModelDefinition) ([]types.Change, error) {
	...
	for tableName, currentDef := range currentMap {
		...
		if !exists {
			...
			changes = append(changes, &types.CreateTable{Table: currentDef})
			
			// Add indexes for new table
			for _, idx := range currentDef.Meta.Indexes {
				changes = append(changes, &types.AddIndex{
					Table: tableName,
					Index: idx,
				})
			}
			...
			for _, rel := range currentDef.Relations {
				if rel.Type == "ForeignKey" || rel.Type == "OneToOne" {
					...
					if targetTable != "" {
						changes = append(changes, &types.AddForeignKey{
							Table:       tableName,
							Relation:    rel,
	...
	for tableName := range previousMap {
		if _, exists := currentMap[tableName]; !exists {
			changes = append(changes, &types.DropTable{Table: tableName})
		}
	}
...
func (d *Detector) detectTableChanges(current, previous *generator.ModelDefinition, tableName string, allDefs []*generator.ModelDefinition) ([]types.Change, error) {
	var changes []types.Change
...
func (d *Detector) detectColumnChanges(tableName string, current, previous []generator.FieldDefinition) ([]types.Change, error) {
	var changes []types.Change
	...
	for name, field := range currentMap {
		if _, exists := previousMap[name]; !exists {
			changes = append(changes, &types.AddColumn{
				Table:  tableName,
				Column: field,
			})
		}
	...
	for name := range previousMap {
		if _, exists := currentMap[name]; !exists {
			changes = append(changes, &types.DropColumn{
				Table:      tableName,
				ColumnName: name,
			})
		}
	...
	for name, currentField := range currentMap {
		if previousField, exists := previousMap[name]; exists {
			if d.fieldChanged(currentField, previousField) {
				changes = append(changes, &types.ModifyColumn{
					Table:     tableName,
...
func (d *Detector) detectIndexChanges(tableName string, current, previous []generator.IndexDefinition) ([]types.Change, error) {
	var changes []types.Change
	...
	for name, idx := range currentMap {
		if _, exists := previousMap[name]; !exists {
			...
			if d.indexChanged(idx, prevIdx) {
				changes = append(changes, &types.ModifyIndex{
					Table:   tableName,
					OldIndex: prevIdx,
					NewIndex: idx,
				})
	...
	for name := range previousMap {
		if _, exists := currentMap[name]; !exists {
			changes = append(changes, &types.DropIndex{
				Table:     tableName,
				IndexName: name,
			})
		}
...
func (d *Detector) detectForeignKeyChanges(tableName string, current, previous []generator.RelationDefinition, currentDef, previousDef *generator.ModelDefinition, allDefs []*generator.ModelDefinition) ([]types.Change, error) {
	var changes []types.Change
	...
	for name, rel := range currentMap {
		...
		if _, exists := previousMap[name]; !exists {
			...
			if d.relationChanged(rel, prevRel) {
				...
				changes = append(changes, &types.ModifyForeignKey{
					Table:       tableName,
					OldFK:       prevRel,
					NewFK:       rel,
					TargetTable: targetTable,
	...
	for name := range previousMap {
		if _, exists := currentMap[name]; !exists {
			changes = append(changes, &types.DropForeignKey{
				Table:  tableName,
...
func (d *Detector) detectConstraintChanges(tableName string, current, previous []generator.ConstraintDefinition) ([]types.Change, error) {
	var changes []types.Change
	...
	for name, constr := range currentMap {
		if _, exists := previousMap[name]; !exists {
			changes = append(changes, &types.AddConstraint{
				Table:      tableName,
				Constraint: constr,
			})
		}
	...
	for name := range previousMap {
		if _, exists := currentMap[name]; !exists {
			changes = append(changes, &types.DropConstraint{
				Table:          tableName,
				ConstraintName: name,
			})
		}
