...
type MigrationGenerator struct {
	...
	sqlBuilder    sqlgen.SQLBuilder
	stateManager  schema.StateManager
}
...
func NewMigrationGenerator(
	...
	driver := types.Driver(driverName)
...
func NewMigrationGeneratorWithDefaults(modelsDir, migrationsDir string) (*MigrationGenerator, error) {
	...
	sqlBuilder, err := sqlgen.NewSQLBuilder(driver)
	if err != nil {
	...
	stateManager := schema.NewFileStateLoader(migrationsDir)
...
func (g *MigrationGenerator) GenerateMigrations(name string) error {
	...
	if schemaState == nil {
		schemaState = &schema.SchemaState{
			Tables: make(map[string]*schema.TableState),
		}
	}
	...
	// See verify/checksum.go for checksum validation
...
func validateChanges(changes []types.Change, defs []*generator.ModelDefinition) error {
...
func generateBookkeepingTable(driver types.Driver) string {
	if driver.IsSQLite() {
