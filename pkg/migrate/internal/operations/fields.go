...
func (op *AddField) StateForwards(appLabel string, s *schema.SchemaState) error {
	table, exists := s.Tables[op.ModelName]
	if !exists {
		return fmt.Errorf("table %s does not exist", op.ModelName)
	}
	table.Columns[op.FieldName] = &schema.ColumnState{
		Name:     op.FieldName,
		Type:     op.Field.Type,
...
func (op *AddField) DatabaseForwards(appLabel string, schemaEditor SchemaEditor, fromState, toState *schema.SchemaState) error {
	return schemaEditor.AddColumn(op.ModelName, op.Field)
}

// DatabaseBackwards removes the column from the database
func (op *AddField) DatabaseBackwards(appLabel string, schemaEditor SchemaEditor, fromState, toState *schema.SchemaState) error {
...
func (op *RemoveField) StateForwards(appLabel string, s *schema.SchemaState) error {
	table, exists := s.Tables[op.ModelName]
	if !exists {
		return fmt.Errorf("table %s does not exist", op.ModelName)
	}
...
func (op *RemoveField) DatabaseForwards(appLabel string, schemaEditor SchemaEditor, fromState, toState *schema.SchemaState) error {
	return schemaEditor.RemoveColumn(op.ModelName, op.FieldName)
}

// DatabaseBackwards adds the column back to the database
func (op *RemoveField) DatabaseBackwards(appLabel string, schemaEditor SchemaEditor, fromState, toState *schema.SchemaState) error {
...
func (op *AlterField) StateForwards(appLabel string, s *schema.SchemaState) error {
	table, exists := s.Tables[op.ModelName]
	if !exists {
		return fmt.Errorf("table %s does not exist", op.ModelName)
	}
...
func (op *AlterField) DatabaseForwards(appLabel string, schemaEditor SchemaEditor, fromState, toState *schema.SchemaState) error {
	return schemaEditor.AlterColumn(op.ModelName, op.OldField, op.NewField)
}

// DatabaseBackwards reverts the column alteration
func (op *AlterField) DatabaseBackwards(appLabel string, schemaEditor SchemaEditor, fromState, toState *schema.SchemaState) error {
...
func (op *RenameField) StateForwards(appLabel string, s *schema.SchemaState) error {
	table, exists := s.Tables[op.ModelName]
	if !exists {
		return fmt.Errorf("table %s does not exist", op.ModelName)
	}
...
func (op *RenameField) DatabaseForwards(appLabel string, schemaEditor SchemaEditor, fromState, toState *schema.SchemaState) error {
	return schemaEditor.RenameColumn(op.ModelName, op.OldName, op.NewName)
}

// DatabaseBackwards reverts the column rename
func (op *RenameField) DatabaseBackwards(appLabel string, schemaEditor SchemaEditor, fromState, toState *schema.SchemaState) error {
