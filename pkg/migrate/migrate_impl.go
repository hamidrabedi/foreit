...
func LoadState(migrationsDir string) (*State, error) {
	schemaState, err := schema.LoadStateFromFiles(migrationsDir)
	if err != nil {
		return nil, err
	}
	return &State{
...
func (s *State) ApplyMigration(changes []Change) error {
	manager := schema.NewInMemoryState()
...
func NewSQLGenerator(driver string) (*SQLGenerator, error) {
	...
	builder, err := sqlgen.NewSQLBuilder(driverType)
	if err != nil {
		return nil, err
	}
	return &SQLGenerator{
