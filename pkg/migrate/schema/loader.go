...
import (
	...
	"github.com/forgego/forge/pkg/migrate/internal/sort"
	"github.com/forgego/forge/pkg/migrate/sqlparse"
)
...
func (l *FileStateLoader) Load() (*SchemaState, error) {
	...
	if l.state == nil {
		...
		sortedFiles := sort.SortMigrationFiles(matches)

		// Parse each migration file and apply to state
		parser := sqlparse.NewSQLParser()
...
func (l *FileStateLoader) Apply(changes []types.Change) error {
...
func applyChangeToState(state *SchemaState, change types.Change) error {
	// Create a temporary manager with the state
	manager := &InMemoryState{state: state}
	return manager.Apply([]types.Change{change})
}
