...
import (
	...
	"github.com/forgego/forge/pkg/migrations/utils"
)
...
func NewDDLParser() *DDLParser {
	return NewDDLParserWithDialect(DialectPostgres)
}

// NewDDLParserWithDialect creates a new DDL parser with a specific dialect
func NewDDLParserWithDialect(dialect Dialect) *DDLParser {
	return &DDLParser{
		dialect:     dialect,
...
func (p *DDLParser) parseAddForeignKey(sql string) *core.AddForeignKey {
	...
	if len(matches) > 6 && matches[6] != "" {
		onDelete = utils.NormalizeCascadeAction(matches[6])
	}
	
	onUpdate := "NO ACTION"
	if len(matches) > 7 && matches[7] != "" {
		onUpdate = utils.NormalizeCascadeAction(matches[7])
	}
	
	relation := generator.RelationDefinition{
		Name: columnName,
		Options: map[string]interface{}{
			"on_delete": utils.DenormalizeCascadeAction(onDelete),
			"on_update": utils.DenormalizeCascadeAction(onUpdate),
		},
	}
