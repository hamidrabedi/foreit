...
// LegacyPlugin is the old plugin interface (deprecated, use registry.Plugin instead)
type LegacyPlugin interface {
	Extension
	Install(app interface{}) error
}

// RegisterLegacyPlugin registers a legacy plugin (deprecated)
func RegisterLegacyPlugin(plugin LegacyPlugin) error {
	if err := RegisterExtension(plugin); err != nil {
		return err
	}

	// TODO: Call Install when app is available
	return nil
}
